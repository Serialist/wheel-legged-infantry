...
whxKalmanFilter_t vaEstimateKF; // 这是一个卡尔曼滤波
...
float vaEstimateKF_Q[4] = { 0.4f, 0.0f, 
							0.0f, 0.5f }; // 过程 Q 矩阵 初始值

float vaEstimateKF_R[4] = { 80.0f, 0.0f,
							 0.0f, 100.0f }; // 观测 R 矩阵 初始值

const float vaEstimateKF_H[4] = { 1.0f, 0.0f, 
								0.0f, 1.0f }; // 观测矩阵 H 为常量
// clang-format on
...
extern "C" void Observer_Task(void const* argument) {
    ...
    while (1) {
        ...
        motor_vel_r = HEXROLL_VELOCITY(m3508[RIGHT].velocity);
        motor_vel_l = -HEXROLL_VELOCITY(m3508[LEFT].velocity);

        // 机体速度观测器

        // 右
        wr = motor_vel_r          // 轮毂反馈速度
            + ins.Gyro[0]         // 机体角速度
