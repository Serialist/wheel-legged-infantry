...
void Chassis_Motor_Transmit(void)
{
	...
	/// @brief 用 3508
	RM_Motor_Control_Transmit(&hcan1,
							  M3508_TX_ID_1,
...
void Jump_FSM(void)
{
	...
	case JPS_INIT:
	{
		...
		if (jump_time >= init_time)
		{
			...
			PID_Reset(&length_pid[LEFT]);
			PID_Reset(&length_pid[RIGHT]);
	...
	case JPS_STRETCH:
	{
			...
			jump_time >= stretch_time)
		{
			...
			PID_Reset(&jump_length_pid[LEFT]);
			PID_Reset(&jump_length_pid[RIGHT]);
	...
	case JPS_STRETCH_DAMPING:
	{
		...
		{
			PID_Reset(&damping_pid[LEFT]);
			PID_Reset(&damping_pid[RIGHT]);
